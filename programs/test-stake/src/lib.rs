...
pub mod test_stake {
    ...
    pub fn stake_nft(ctx: Context<StakeNFT>, symbol: String, period: i64) -> ProgramResult {
        ...
        if period != 5184000 || period != 7776000 {
            return Err(ErrorCode::NoPeriodTime.into())
        }
    ...
    pub fn reclaim_nft(ctx: Context<ReclaimNFT>) -> ProgramResult {
        ...
        let mut founded = None;
        {
            ...
            let period = list.items.get(founded.unwrap()).unwrap().period_time;

            if founded.is_some()
                && ctx.accounts.clock.unix_timestamp
                < period.clone() + list.items.get(founded.unwrap()).unwrap().stake_time
            {
            ...
            let real_per = period.clone() / 24 / 60 / 60;
